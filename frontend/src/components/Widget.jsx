...
// Build WS URL from current host — goes through Vite proxy in dev,
// or connects directly in production.
...
export default function Widget() {
  ...
  // ── Handle messages from voice_pipeline.py ───────────────────────
  const handleMessage = useCallback((evt) => {
    // Binary = base64-decoded TTS audio bytes from Cartesia/DeepGram
    ...
    switch (msg.type) {
      ...
      // ✅ Backend sends 'session' as the first frame after WS connects
      // Fields: session_id, driver_id, interaction_id
      case 'session':
        setSessionId(msg.session_id)
        setDriverName(msg.driver_id)   // show driver_id until driver info loads
        setMode('logistics')
        setCallState(STATE.LIVE)
        setStatusMsg(`Session active — ${msg.session_id}`)
        startRecording()
        break

      // STT transcript from DeepGram Nova 3
      case 'transcript':
        setStatusMsg(`You: ${msg.text}`)
        break

      // LLM response text from Groq/Llama
      case 'response':
        setStatusMsg(`ANAIRA: ${msg.text}`)
        break

      // TTS audio as base64 string from Cartesia/DeepGram fallback
      case 'audio': {
        try {
          ...
          console.error('[Widget] audio decode error', e)
        }
        break
      }

      // Tool call completed (informational only)
      case 'tool_call':
      ...
      // Pipeline done for this turn — back to listening
      case 'done':
  ...
  }, [playChunk, startRecording]) // endCall added below via ref

  // ── Open WebSocket — no REST call needed ─────────────────────────
  const startCall = useCallback(() => {
    setError(null)
    setCallState(STATE.CONNECTING)
    setStatusMsg('Connecting…')

    // ✅ FIXED: connect directly — backend creates session on WS open
    const ws = new WebSocket(WS_URL)
    ws.binaryType = 'arraybuffer'
    wsRef.current = ws

    ws.onopen = () => {
      // Backend immediately sends { type: 'session', session_id, driver_id, interaction_id }
      // No JSON handshake needed from the client side
      setStatusMsg('Waiting for session…')
    }
    ...
    ws.onclose = (evt) => {
      if (callState === STATE.LIVE || callState === STATE.CONNECTING) {
        setCallState(STATE.IDLE)
        setStatusMsg('')
      }
    }
  }, [handleMessage, callState])

  // ── End call — close WS cleanly, no /session/end REST call ───────
  const endCall = useCallback(() => {
    ...
    if (wsRef.current) {
      // Closing the WebSocket is sufficient — backend handles cleanup
      // in voice_pipeline.py's WebSocketDisconnect handler
  ...
  useEffect(() => {
    return () => {
      wsRef.current?.close(1000, 'Widget unmounted')
    }
  }, [])
      ...
      <div style={styles.card} className="glass-card glow-violet card-3d">
        ...
        <div style={styles.header}>
          ...
          <div style={styles.modeTag}>
            {mode ? (
              ...
              <span className="badge badge-violet">v2.0</span>
            )}
          </div>
        </div>
        ...
        <div style={styles.statusArea}>
          {callState === STATE.IDLE && (
            <p style={styles.statusText}>Tap to start a voice session</p>
          )}
          ...
          {isLive && (
            <div style={{ textAlign: 'center' }}>
              ...
              {statusMsg && (
                <p style={{ ...styles.statusText, fontSize: 12, marginTop: 6 }}>{statusMsg}</p>
              )}
            </div>
          )}
          {callState === STATE.ENDING && (
            <p style={styles.statusText}>Ending session…</p>
          )}
        </div>
...
const styles = {
  ...
  pulseRing: {
    ...
    height: 120,
    borderRadius: '50%',
    border: '2px solid rgba(20,217,196,0.5)',
